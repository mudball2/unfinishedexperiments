...
void CSGD_Direct3D::ChangeDisplayParam(int nWidth, int nHeight, bool bWindowed)
{
	...
	if( bWindowed == false )
	{
		...
		IDirect3D9* ptr;
		m_lpDirect3DDevice->GetDirect3D( &ptr );
		...
		for( int i = 0; i < size; i++ )
		{
			...
			if( nWidth == mode.Width && nHeight == mode.Height )
			{
				valid = true;
				break;
			}
			else if( nWidth <= mode.Width && nHeight <= mode.Height )
			{
				if( mode.Width <= nLargerWidth && mode.Height <= nLargerHeight )
				{
					larger = true;
					nLargerWidth  = mode.Width;
					nLargerHeight = mode.Height;
				}
			}
		}
...
///////////////////////////////////////////////////////////////////
void CSGD_Direct3D::DrawText(const TCHAR *lpzText, int nX, int nY, unsigned char ucRed, unsigned char ucGreen, unsigned char ucBlue)
{
