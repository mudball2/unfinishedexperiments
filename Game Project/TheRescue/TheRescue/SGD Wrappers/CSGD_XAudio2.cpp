...
void CSGD_XAudio2::Update()
{
	...
	//TCHAR text[64];
	//_stprintf_s( text, 64, _T("%i"), m_vVoices.size() );
...
void CSGD_XAudio2::RecycleVoiceNow(int nIndex)
{
	...
	if (iter == m_vVoicePools.end())
	{
		...
		TCHAR buffer[128];
		_stprintf_s(buffer, _countof(buffer), _T("RecycleVoiceNOW::Shouldn't EVER get here: Index %d VoicePool %d\n"));
...
int CSGD_XAudio2::SFXLoadSound( const TCHAR* szFileName )
{
	...
	if (len > 4)
	{
		...
		const TCHAR* pExtention = (szFileName + (len-4));

		if (_tcsicmp(pExtention, _T(".xwm")) == 0)
		{
	...
	_tcscpy_s(m_vSFXLibrary[nID].filename, _countof(m_vSFXLibrary[nID].filename), szFileName);
...
int CSGD_XAudio2::MusicLoadSong( const TCHAR* szFileName )
{
	...
	if (len > 4)
	{
		...
		if (_tcsicmp(pExtention, _T(".wav")) == 0)
		{
	...
	_tcscpy_s(m_vMusicLibrary[nID].filename, _countof( m_vMusicLibrary[nID].filename ), szFileName);
